...
import { ODER_STATUS, ODER_STATUS_STRING, TRANSFER_PRICE } from "../../src/constants";
...
interface Order {
  ...
  price: number;
  quantity: number; // Fixed typo here from 'quanity' to 'quantity'
  sizeName: string;
}

export default function OrderDetails() {
  const [lstOders, setLstOders] = useState<Order[]>([]);
  ...
  useEffect(() => {
    (async () => {
      try {
        const response = await AxiosRequest.post<{ data: Order[] }>("/orders/lst-orders", {
          status: orderStatus,
        });
        setLstOders(response.data.data || []);
      } catch (error) {
  ...
  const items = ODER_STATUS_STRING.map((status) => ({
    ...
    children: (
      <div className="flex-1 mt-6 space-y-8">
        {lstOders.length > 0 ? (
          lstOders.map((order) => (
            <div key={order.productId}> {/* Use a unique identifier */}
